...
Object3D::Object3D(vector<vec3>& points, vector<vec3>& colors, vector<int>& indices) : Object3D() {
...
void Object3D::update(vector<vec3>& points, vector<vec3>& colors, vector<int>& indices) {
    size = indices.size();
...
void PointCloudGraphics::allocate() {
    glGenVertexArrays(1, &vaoId);
    glGenBuffers(1, &pointsGPU);
}

PointCloudGraphics::~PointCloudGraphics() {
    glDeleteVertexArrays(1, &vaoId);
    glDeleteBuffers(1, &pointsGPU);
}
...
PointCloudGraphics::PointCloudGraphics(PointCloudGraphics&& other) noexcept {
    swap(other);
}

PointCloudGraphics& PointCloudGraphics::operator=(PointCloudGraphics other) {
    swap(other);
    return *this;
}

void PointCloudGraphics::swap(PointCloudGraphics& other) {
    std::swap(size, other.size);
    std::swap(vaoId, other.vaoId);
    std::swap(pointsGPU, other.pointsGPU);
}
    ...
    if (action == GLFW_PRESS) {
        switch (key) {
            case GLFW_KEY_LEFT_BRACKET: {
                viewer->frame = max(0, viewer->frame - 1);
                break;
            }
            case GLFW_KEY_RIGHT_BRACKET: {
                viewer->frame = min(viewer->maxFrame - 1, viewer->frame + 1);
                break;
            }
            case GLFW_KEY_SPACE: {
...
void Viewer::addObject(Object3D&& obj, bool ephemeral) {
    if (ephemeral) ephemeralObjects.push_back(move(obj));
    else objects.push_back(move(obj));
}
...
void Viewer::updatePointCloud(PointCloud& pc) {
    // Calculate bounds
    ...
    pcCenter = vec3((minX + maxX) / 2, 0.0f, (minZ + maxZ) / 2);
    pcGraphics.update(pc);
}
